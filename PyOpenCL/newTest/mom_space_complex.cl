...
float2 get_element(int x, float2 start, float2 end, int n)
{
    ...
    for (int i=0;i<25;i++)
        sum = sum + c_mul(to_r(get_gauss_legendre_w_25(i)) , (integrand(((end-start)/2.0f*get_gauss_legendre_x_25(i) + (start+end)/2.0f), (xx), (yy), (step))));
...
float2 get_element_berggren(float2 start, float2 end, float2 step, float2 xx, float2 yy)
{
    // return 1.0f;
    start=c_real(start); // Needed
    end=c_real(end);     // Needed
    float mass=0.019272f;
    float2 sum=to_r(0.0f);
    for (int i=0;i<25;i++)
        sum = sum + c_mul(to_r(get_gauss_legendre_w_25(i)) , (integrand(((end-start)/2.0f*get_gauss_legendre_x_25(i) + (start+end)/2.0f), (xx), (yy), (step))));
